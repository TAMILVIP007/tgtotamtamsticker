...
class TamTamBot:
    ...
    def api_request(self, method: str, params: dict = None):
        ...
        res = requests.get(f"{self.base_url}/{method}",
                           params=params)  # type: requests.Response
    ...
    def proceed_message_created(self, data):
        ...
        if tg_set is None:
            self.send_message(
                ...
                MSG_SET_NOT_FOUND.format(tg_set_name=tg_set_name),
                use_markdown=True,
            )
            return web.Response()
        ...
        self.send_message(
            update.message.sender.user_id,
            MSG_SET_IN_PROGRESS.format(tg_set_name=tg_set_name),
        )
        try:
    ...
    def run(self):
        ...
        web.run_app(app, port=int(os.environ.get("PORT", 443)))
